...
- [ ] Long arrow appears only when `LC OR Break Above` prints and long filters pass.
- [ ] Short arrow appears only when `LC OR Break Below` prints and short filters pass.
- [ ] Stops track selected external levels (`LC Bull IFVG Bottom` / `LC Bear IFVG Top`).

Here is the highly technical, constraint-heavy prompt you can copy and paste directly into Gemini 3.1 Pro. It integrates the QA resolutions from our previous conversation, the specific Prop Firm rules from the Master Blueprint, and the rigorous memory and syntax optimizations from the newly synthesized Pine Script v6 research.
...
**Task:** Write **ONLY** the Pine Script v6 code for **Phase \[2\]** of the "God Mode" Multi-Timeframe NQ Scalping Engine. You already have the Master Blueprint framework. **Do NOT write the code for any other phases, and do NOT provide placeholder comments for the rest of the script.** Your output should only contain the specific module requested.
...
**4\. Institutional Execution Handicaps:** Strategy declarations must strictly use the Master Blueprint's cloud-routed handicaps: `slippage = 2`, `commission_value = 2.50`, `commission_type = strategy.commission.cash_per_contract`, `use_bar_magnifier = true`, and `process_orders_on_close = true`.
...
**6\. Cycle Memory & State Tracking (Ban `ta.barssince`):** Do NOT use `ta.barssince()`, as it crashes the MTF engine by forcing deep historical scans. Use single-threaded native `var` declarations for state tracking. Establish immutable macro-anchors (`bar_index` and `price`) the moment a structural event occurs. Use forward-rolling counters (`var int count = na; count := condition ? 0 : count + 1`) and calculate distances natively via `bar_index - anchorBar`.

**7\. The `f_eval` Master Parser Routing:** The `f_eval` function must act as a universal translator using `switch` blocks to route UI timeframe strings ("HTF 1", "HTF 2", "HTF 3") to their respective tuple arrays (`h1_`, `h2_`, `h3_` prefixes). It must accept a `dir` argument (1 for Long, \-1 for Short) to automatically align directional logic across timeframes.
...
**Output:** Provide ONLY the fully optimized Pine Script v6 code block for **Phase \[2\]** with brief inline technical comments explaining how the constraints were satisfied.

Here is the highly technical, constraint-heavy prompt you can copy and paste directly into Gemini 3.1 Pro. It integrates the rigorous memory optimization syntax from the latest Pine Script v6 research, the QA resolutions from our execution audit, and the explicit Prop Firm rules from the Master Blueprint.
...
**Task:** Write **ONLY** the Pine Script v6 code for **Phase \[4\]** of the "God Mode" Multi-Timeframe NQ Scalping Engine. You already have the Master Blueprint framework. **Do NOT write the code for any other phases, and do NOT provide placeholder comments for the rest of the script.** Your output should strictly contain the specific module requested.
...
**7\. The `f_eval` Master Parser Routing:** The `f_eval` function must act as a universal translator using `input.enum()` and `switch` blocks to eliminate deep `if...else` chains. It must route UI timeframe strings ("HTF 1", "HTF 2", "HTF 3") to their respective tuple arrays (`h1_`, `h2_`, `h3_` prefixes). It must accept a `dir` argument (1 for Long, \-1 for Short) to automatically align directional logic across timeframes.
...
**Output:** Provide ONLY the fully optimized Pine Script v6 code block for **Phase \[4\]** with brief inline technical comments explaining how the constraints were satisfied.
